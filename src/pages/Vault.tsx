...
const Vault: React.FC = () => {
    ...
    React.useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                // Show course-like items (course, devlog, session) that are in the vault destination
                const vaultOnly = data.filter(item => {
    ...
    // Calculate counts for all options based on current filtered set (or initial set)
        ...
        <Layout>
            <div className="pt-24 pb-20 px-4 md:px-8 min-h-screen max-w-[1800px] mx-auto">
                <div className="flex flex-col md:flex-row relative">
                    ...
                    <div className="flex-1">
                        <div className="mb-6 flex items-center justify-end">
                            <div className="flex items-center gap-4">
                                <p className="text-zinc-500 text-sm">{filtered.length} Items</p>
                                <button
                        ...
                        {loading ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-20">
                                {filtered.map(item => (
                                    <div key={item.id}>
                                        <VideoCard content={item} onClick={() => { setSelected(item); setShowModal(true); }} variant="fluid" />
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center py-20 bg-zinc-900/30 rounded-3xl border border-dashed border-zinc-800">
                                <p className="text-zinc-500 mb-2">No items match your selected filters.</p>
                                <button
                    ...
                    {selected && (
                        <CourseDetailsModal content={selected} isOpen={showModal} onClose={() => { setShowModal(false); setSelected(null); }} />
                    )}
                </div>
            </div>
        </Layout>
